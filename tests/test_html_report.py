...
def fake_home(tmp_path):
    ...
    (home / "Legal").mkdir()
    (home / "Human Review").mkdir()
    (home / "Downloads" / "example.txt").write_text("hello")
    return home


def test_generate_report_creates_file(tmp_path, sample_state, fake_home):
    output = tmp_path / "home-index.html"
    generate_report(sample_state, str(fake_home), str(output))
    assert output.exists()


def test_report_contains_summary_stats(tmp_path, sample_state, fake_home):
    output = tmp_path / "home-index.html"
    generate_report(sample_state, str(fake_home), str(output))
...
def test_report_contains_move_log(tmp_path, sample_state, fake_home):
    output = tmp_path / "home-index.html"
    generate_report(sample_state, str(fake_home), str(output))
    html = output.read_text()
...
def test_report_contains_amber_section(tmp_path, sample_state, fake_home):
    output = tmp_path / "home-index.html"
    generate_report(sample_state, str(fake_home), str(output))
    html = output.read_text()
...
def test_report_contains_file_links(tmp_path, sample_state, fake_home):
    output = tmp_path / "home-index.html"
    generate_report(sample_state, str(fake_home), str(output))
    html = output.read_text()
    assert "file://" in html
...
def test_report_no_amber_section_when_queue_empty(tmp_path, sample_state, fake_home):
...
def test_report_escapes_xss_in_filenames(tmp_path, sample_state, fake_home):
...
def test_report_escapes_xss_in_ai_reason(tmp_path, sample_state, fake_home):
...
def test_report_shell_safe_mv_command(tmp_path, sample_state, fake_home):
